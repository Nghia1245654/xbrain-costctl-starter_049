...
def _region_name_fallback():
    """Return the configured region, falling back to us-east-1.

    In CI/tests (moto) there's often no AWS config, so boto3 can raise
    NoRegionError unless we provide a region.
    """
...
def _list_ec2(want, missing):
    ...
    """
    ec2 = boto3.client("ec2", region_name=_region_name_fallback())
...
def _list_rds(want, missing):
    ...
    """
    rds = boto3.client("rds", region_name=_region_name_fallback())
    rows = []
...
def _list_s3(want, missing):
    ...
    """
    s3 = boto3.client("s3", region_name=_region_name_fallback())
    rows = []
    for bucket in s3.list_buckets().get("Buckets", []):
        ...
        try:
            ...
            tags = {t.get("Key"): t.get("Value") for t in resp.get("TagSet", [])}
        except ClientError:
            # No tagging config → treat as empty tags (per docstring).
            tags = {}
...
def run(args):
    ...
    tag_bits = []
    tag_bits.extend([f"{k}={v}" for k, v in want])
    tag_bits.extend([f"missing:{k}" for k in missing])
    tag_desc = " ".join(tag_bits) if tag_bits else "(no filters)"

    print(f"{args.type.upper()} {tag_desc} — {len(rows)} found:")
    print("-" * 78)
