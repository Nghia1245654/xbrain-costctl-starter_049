...
def _find_targets(tag_key, tag_val):
    ...
    # EC2 instances: include only non-terminal states.
    ...
    # EBS volumes: only delete 'available' volumes (skip in-use).
...
def run(args):
    ...
    if not args.apply:
        print(
            f"Plan: terminate {len(ec2_ids)} EC2 + delete {len(volume_ids)} volume(s) "
            f"(dry-run — pass --apply to execute)"
        )
        return
